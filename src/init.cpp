...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("SONO version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  sonod [options]                     " + "\n" +
                  "  sonod [options] <command> [params]  " + _("Send command to -server or sonod") + "\n" +
                  "  sonod [options] help                " + _("List commands") + "\n" +
                  "  sonod [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "sono:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("SONO"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("SONO"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: sono.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: sonod.pid)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 32000 or testnet: 29000)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SONO is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  SONO is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("SONO version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "SONO server starting\n");
        ...
        if(nWalletBackups > 0)
        {
            if (filesystem::exists(backupDir))
            {
                ...
                try {
                    ...
                    printf("Creating backup of %d -> %d\n", sourceFile, backupFile);
                } catch(boost::filesystem::filesystem_error &error) {
                    printf("Failed to create backup %d\n", error.what());
                }
    ...
    if (!bitdb.Open(GetDataDir()))
    {
        string msg = strprintf(_("Error initializing database environment %s!"
                                 " To recover, BACKUP THAT DIRECSONOY, then remove"
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("SONO"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (!(mapArgs.count("-tor") && mapArgs["-tor"] == "0") && (fProxy || mapArgs.count("-tor"))) {
        ...
        SetProxy(NET_SONO, addrOnion, 5);
        SetReachable(NET_SONO);
    }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of SONO") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart SONO to complete") << "\n";
    ...
    if (!CheckDiskSpace())
    {
        return InitError(_("Error: not enough disk space to start SONO."));
    }
