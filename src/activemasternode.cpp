...
vector<COutput> CActiveMasternode::SelectCoinsMasternode(bool fSelectUnlocked)
{
    ...
    {
        if(out.tx->vout[out.i].nValue == GetMNCollateral()*COIN) { //exactly 5,000 SONO
...
vector<COutput> CActiveMasternode::SelectCoinsMasternodeForPubKey(std::string collateralAddress)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        if(out.tx->vout[out.i].scriptPubKey == scriptPubKey && out.tx->vout[out.i].nValue == GetMNCollateral()*COIN) { //exactly 5,000 SONO
            filteredCoins.push_back(out);
        }
    }
    return filteredCoins;
