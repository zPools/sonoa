...
const string strMessageMagic = "SONO Signed Message:\n";
...
{
    if (pindexBest == NULL || nBestHeight < Checkpoints::GetTotalBlocksEstimate() || nBestHeight < (GetNumBlocksOfPeers() - 60))
        return true;
        ...
        if(IsProofOfStake() && pindex != NULL){
            if(pindex->GetBlockHash() == hashPrevBlock){
                ...
                if (pindex != NULL)
                {
                    ...
                    if(fDebug) { printf("CheckBlock-POS(): Transaction 1 Size : %li\n", vtx[1].vout.size()); }
            ...
            if(pindex->GetBlockHash() == hashPrevBlock){
                ...
                if (pindex != NULL)
                {
                    ...
                    // Check transaction for payee and if contains masternode reward payment
                    if(fDebug) { printf("CheckBlock-POW(): Transaction 0 Size : %li\n", vtx[0].vout.size()); }
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // SONO: ask for pending sync-checkpoint if any
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        strMiscWarning = strMessage;
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SONO", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    int64_t ret = static_cast<int64_t>(blockValue * 1/4); //25%
